...
open class SurveyQuestions {
    ...
    func submitTitle() -> String {
        return self.submitData["button_title"] ?? "Submit Data"
    }
    ...
    class open func loadFromJsonObject(dict: [[String: Any?]], surveyTheme: SurveyTheme) -> SurveyQuestions? {
        ...
        if dict.count > 0 {
            do {
                ...
                loadedQuestions = SurveyQuestions(dict as! [[String : Any?]], submitData: submitData, surveyTheme: surveyTheme)
                
                if let autoFocus = dict[0]["auto_focus_text"] as? Bool {
                    loadedQuestions?.autoFocusText = autoFocus
                }
            } catch {
                Logger.log(error.localizedDescription, level: .error)
